...
const createTomas = async () => {
  const tomasDefault = [
    {
      nombre: "TOMOGRAFÍA 1MAXILAR USB",
      precio_standart: 40,
    },
    {
      nombre: "TOMOGRAFÍA BIMAXILAR USB",
      precio_standart: 40,
    },
  ];
  tomasDefault.forEach(async (t) => {
    const newToma = new Toma({
      nombre: t.nombre,
      precio_standard: t.precio_standart,
    });
    await newToma.save();
    console.log("Nueva toma creada", newToma.nombre);
  });
};
...
const ordenarTomas = async () => {
  await Toma.findByIdAndDelete('647b643e9f68ce43944ac756');
};
const deleteOrdenes = async () => {
  await Orden.deleteMany();
  console.log("ordenes eliminadas");
};

const editBoletas = async () => {
  const ordenes = await Orden.find();
  ordenes.forEach(async (o) => {
    o.boletaa = o.boleta;
    await o.save();
    return console.log(o.boleta);
  });
};

export { deleteOrdenes, createTomas, ordenarTomas, editBoletas };
